/*
Logic: visit each city using DFS, maintain a hash map of
city: pair<stop,price> (the stop and price to reach certain city)
if stop is higher than our limit or the price is higher than the current optimal result price, we won't visit the next city
if we revisit a city, we will check whether there is already a more optimal route exisiting - meaning
1. a route with fewer stops && same or lower price.
2. a route with same number of stops but cheaper or same price.
if there is a more optimal route existing, we will not continue with this route

possible improvement:if you change the unordered_map<int,vector<pair<int,int>>> stop_price into
unordered_map<int,vector> and get the element by index, the performance would be better
*/
class Solution {
public:
